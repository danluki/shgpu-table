...
const tableApi = new TableAPI(process.env.API_URL);
const bot = new TelegramBot(process.env.TELEGRAM_TOKEN, { polling: true });
tableApi.addListener("tableCreated", async (data) => {
    const { tableWeek, faculty, link } = JSON.parse(data);
    const subs = await repository.getFacultySubscribers(faculty.id);
    const date = tableWeek.beginDate > new Date() ? "следущую" : "текущую";
...
tableApi.addListener("tableUpdated", async (data) => {
    const { tableWeek, faculty, link } = JSON.parse(data);
    const subs = await repository.getFacultySubscribers(faculty.id);
    const date = tableWeek.beginDate > new Date() ? "следущую" : "текущую";
...
async function start() {
    ...
    bot.onText(/\/начать/, (msg) => {
        bot.sendMessage(msg.chat.id, "Добро пожаловать в неофицального ШГПУ бота с расписанием", {
            reply_markup: {
                keyboard: [
    ...
    bot.onText(/Пары на неделю/gi, async (msg) => {
        try {
            ...
            if (subscriber) {
                ...
                bot.sendMessage(msg.chat.id, "Вы не подписаны не на одну из групп, пожалуйста, воспользуйтесь полной версией команды");
            }
        }
        catch (e) {
    ...
    bot.onText(/Пары \S{1,} на неделю/gi, async (msg) => {
    ...
    bot.onText(/Пары на след неделю/gi, async (msg) => {
        try {
            const subscriber = await repository.getSubscriberByChatId(msg.chat.id);
            if (subscriber) {
                const pairs = await tableApi.getWeekPairs(subscriber.group_name, false);
                if (!pairs.length) {
                    bot.sendMessage(msg.chat.id, "😱 Нет информации о парах на следущую неделю");
                    return;
                }
                const pairsMessages = formatPairs(pairs);
                for (const mes of pairsMessages) {
    ...
    bot.onText(/Пары \S{1,} на след неделю/gi, async (msg) => {
        const groupName = msg.text.split(" ")[1];
        try {
            const group = await tableApi.getGroup(groupName);
    ...
    bot.onText(/Пары на завтра/gi, async (msg) => {
        try {
            const subscriber = await repository.getSubscriberByChatId(msg.chat.id);
            if (subscriber) {
    ...
    bot.onText(/Пары \S{1,} на завтра/gi, async (msg) => {
        const groupName = msg.text.split(" ")[1];
        try {
            const group = await tableApi.getGroup(groupName);
            ...
            else if (err instanceof ApiError && err.code === 500) {
                bot.sendMessage(msg.chat.id, `Не удалось подписаться на группу ${groupName}. Ошибка сервера.`);
            }
            console.log(err);
        }
    });
    bot.onText(/Пары на сегодня/gi, async (msg) => {
        try {
            const subscriber = await repository.getSubscriberByChatId(msg.chat.id);
            if (subscriber) {
    ...
    bot.onText(/Пары \S{1,} на сегодня/gi, async (msg) => {
        const groupName = msg.text.split(" ")[1];
        try {
            const group = await tableApi.getGroup(groupName);
    ...
    bot.onText(/Подпиши на \S{1,}/, async (msg, matches) => {
        ...
        tableApi
            ...
            repository
                ...
                if (err instanceof ChatIsAlreadySubscribedError) {
                    bot.sendMessage(msg.chat.id, `Не удалось подписаться на группу ${groupName}. Сначала отпишитесь от текущей.`);
                    return;
                }
                    ...
                    bot.sendMessage(msg.chat.id, `Не удалось подписаться на группу ${groupName}.`);
                    console.log(err);
                    return;
                }
            });
    ...
    //     );
    //   });
    bot.onText(/Скачать/gi, async (msg) => { });
    bot.onText(/Звонки/gi, async (msg) => {
        try {
            ...
            if (e instanceof ApiError && e.code === 500) {
                bot.sendMessage(msg.chat.id, `Не удалось получить расписание. Ошибка сервера.`);
            }
