...
describe("Downloading tables test", () => {
  ...
  const testLink =
    "https://shgpi.edu.ru/fileadmin/rasp/faculty/f11/14_11_2022_20_11_2022/14_11_2022_20_11_2022.xls";
  ...
  test("Invalid link 1", async () => {
    await expect(
      downloadTable("testLink", getFacultyFromLink(testLink))
    ).rejects.toThrow(DownloadingTableError);
  });

  test("Invalid link 2", async () => {
    expect(
      downloadTable(
        "https://shgpi.edu.ru/fileadmin/rasp/faculty/f11/14_11_2022_20_11_2022/testLink.xls",
        getFacultyFromLink(testLink)
      )
    ).rejects.toThrow(DownloadingTableError);
  });
