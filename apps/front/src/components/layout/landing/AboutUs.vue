<template>
  <section class="flex flex-col h-screen m-auto">
    <div class="p-7">
      <h2 class="text-white font-bold mb-5 text-5xl text-center">О нас</h2>
      <div class="gap-4 grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2">
        <div class="duration-700 hover:-translate-y-2">
          <div class="bg-[#181818] block drop-shadow-xl rounded">
            <div class="p-6">
              <h3 class="font-bold mb-4 text-left text-xl">Телеграм бот</h3>
              <h5 class="font-medium text-left text-lg text-white">
                Получи доступ ко всем функциям системы, используя любимый
                мессенджер!
              </h5>
            </div>
          </div>
        </div>
        <div class="duration-700 hover:-translate-y-2">
          <div class="bg-[#181818] block drop-shadow-xl rounded">
            <div class="p-6">
              <h3 class="font-bold mb-4 text-left text-xl">Веб клиент</h3>
              <h5 class="font-medium text-left text-lg text-white">
                Получи доступ ко всем функциям системы, используя любой браузер,
                с поддержкой JavaScript!
              </h5>
            </div>
          </div>
        </div>
        <div class="duration-700 hover:-translate-y-2">
          <div class="bg-[#181818] block drop-shadow-xl rounded">
            <div class="p-6">
              <h3 class="font-bold mb-4 text-left text-xl">
                Мобильное приложение
              </h3>
              <h5 class="font-medium text-left text-lg text-white">
                Получи доступ ко всем функциям системы, используя наше мобильное
                приложение! (В разработке)
              </h5>
            </div>
          </div>
        </div>
        <div class="duration-700 hover:-translate-y-2">
          <div class="bg-[#181818] block drop-shadow-xl rounded">
            <div class="p-6">
              <h3 class="font-bold mb-4 text-left text-xl">Бот Вконтакте</h3>
              <h5 class="font-medium text-left text-lg text-white">
                Получи доступ ко всем функциям системы, используя Вконтакте! (В
                разработке)
              </h5>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="p-7">
      <h2 class="text-white font-bold mb-5 text-5xl text-center">
        Возможности
      </h2>
      <div class="gap-4 grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2">
        <div class="duration-700 hover:-translate-y-2">
          <div class="bg-[#181818] block drop-shadow-xl rounded">
            <div class="p-6">
              <h3 class="font-bold mb-4 text-left text-xl">
                Обновление расписания
              </h3>
              <h5 class="font-medium text-left text-lg text-white">
                Каждые два часа мы проверяем расписание на изменения и сообщаем
                вам об этом. 🚀
              </h5>
            </div>
          </div>
        </div>
        <div class="duration-700 hover:-translate-y-2">
          <div class="bg-[#181818] block drop-shadow-xl rounded">
            <div class="p-6">
              <h3 class="font-bold mb-4 text-left text-xl">
                Поиск по преподавателю
              </h3>
              <h5 class="font-medium text-left text-lg text-white">
                Мы можем найти пары даже по части фамилии преподавателя. Даже
                если он работает на нескольких факультетах
              </h5>
            </div>
          </div>
        </div>
        <div class="duration-700 hover:-translate-y-2">
          <div class="bg-[#181818] block drop-shadow-xl rounded">
            <div class="p-6">
              <h3 class="font-bold mb-4 text-left text-xl">
                Пары на следущую неделю
              </h3>
              <h5 class="font-medium text-left text-lg text-white">
                Как только на сайте опубликуют расписание на следущую неделю вы
                получите уведомление.
              </h5>
            </div>
          </div>
        </div>
        <div class="duration-700 hover:-translate-y-2">
          <div class="bg-[#181818] block drop-shadow-xl rounded">
            <div class="p-6">
              <h3 class="font-bold mb-4 text-left text-xl">
                Пары за любую дату
              </h3>
              <h5 class="font-medium text-left text-lg text-white">
                Мы сохраняем расписание в базе, поэтому вы можете посмотреть
                историю пар.
              </h5>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts"></script>

<style scoped></style>
